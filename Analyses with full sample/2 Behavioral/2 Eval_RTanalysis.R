...
# CONDITION MEANS
tapply(corTrials$TargetWord.RT, paste(corTrials$wordVal,corTrials$faceRace, sep="_"), mean)

tapply(corTrials$TargetWord.RT, corTrials$FixArea, mean)
...
ggplot(corTrials, aes(faceRace, TargetWord.RT, fill = wordVal)) +
  ...
  facet_wrap(~FixArea) +
  coord_cartesian(ylim=c(470, 525)) +
...
sink("./Analyses with full sample/2 Behavioral/Model output/1.1 Behavioral_Eval_VaryingInterceptModel.txt")
summary(m4)
coef(m4)
summary(m4a)$AICtab
sink()
...
sink("./Analyses with full sample/2 Behavioral/Model output (noBS)/1.3 Behavioral_Eval_allRandomSlopesForSubjectOnly.txt")
summary(m6)
coef(m6)
summary(m6a)$AICtab
sink()
...
m7 = lmer(TargetWord.RT ~ faceRace * wordVal * FixArea + 
          (faceRace + wordVal|Subject) + 
          (1|TargetWord), data=corTrials)

sink("./Analyses with full sample/2 Behavioral/Model output (noBS)/1.4 Behavioral_Eval_RaceValRandomForSubject.txt")
summary(m7)
coef(m7)
sink()

m8 = lmer(TargetWord.RT ~ faceRace * wordVal * FixArea + 
            (wordVal|Subject) + 
            (1|TargetWord), data=corTrials)

sink("./Analyses with full sample/2 Behavioral/Model output (noBS)/1.5 Behavioral_Eval_ValRandomForSubject.txt")
summary(m8)
coef(m8)
sink()
