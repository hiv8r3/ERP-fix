...
P2dat = read.delim("./Analyses with full sample/ERP quant data/1 Eval dat/Individual averages/Individual quant files/Sub1_P2quant_avgamp.dat", header = T, sep = "\t")
...
for (i in c(2:64,66)) {
  temp = read.delim(paste("./Analyses with full sample/ERP quant data/1 Eval dat/Individual averages/Individual quant files/Sub", i, "_P2quant_avgamp.dat", sep = ""), 
                    header = T, sep = "\t")
...
write.table(P2dat, "./Analyses with full sample/ERP quant data/1 Eval dat/Individual averages/AllSubs_IndivAverages_P2_meanAmp.txt", row.names = F, sep = "\t")
...
# Just reading in each file for each subject and putting them together into one file with all subjects
P2dat.peak = read.delim("./Analyses with full sample/ERP quant data/1 Eval dat/Individual averages/Individual quant files/Sub1_P2quant_peak.dat", header = T, sep = "\t")

# add column for trial condition
condList = rep("Black_eyes", 7) %>%
  append(rep("Black_fore", 7)) %>%
  append(rep("White_eyes", 7)) %>%
  append(rep("White_fore", 7))
P2dat.peak$Condition = condList

# add column for subject number
P2dat.peak$Subject = 1

# rename average amplitude column so there's no problem rbinding
names(P2dat.peak)[6] = "PeakAmp"

# append the rest of the subjects to same data file using a loop
for (i in c(2:64,66)) {
  temp = read.delim(paste("./Analyses with full sample/ERP quant data/1 Eval dat/Individual averages/Individual quant files/Sub", i, "_P2quant_peak.dat", sep = ""), 
                    header = T, sep = "\t")
  temp$Condition = condList
  temp$Subject = i
  names(temp)[6] = "PeakAmp"
  P2dat.peak = rbind(P2dat.peak, temp)
}

P2dat.peak = rename(P2dat.peak, Electrode = Channel) %>%
  select(-Sweep.Number)

# add column specifying race of prime
P2dat.peak$faceRace = NA
P2dat.peak$faceRace[grep("Black", P2dat.peak$Condition)] = "Black"
P2dat.peak$faceRace[grep("White", P2dat.peak$Condition)] = "White"

# add column specifying fixation area
P2dat.peak$FixArea = NA
P2dat.peak$FixArea[grep("eyes", P2dat.peak$Condition)] = "eyes"
P2dat.peak$FixArea[grep("fore", P2dat.peak$Condition)] = "fore"

write.table(P2dat.peak, "./Analyses with full sample/ERP quant data/1 Eval dat/Individual averages/AllSubs_IndivAverages_P2_peakAmp.txt", row.names = F, sep = "\t")
