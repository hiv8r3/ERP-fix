...
for (k in c(1:64,66)) {
  ...
  for (y in condList) {
    ...
    # Make average waveform for each bin (in condition loop, so this is only for one condition)
    # Where peak amp for each bin will go
    ...
    for (i in 1:numBins) {  # go one bin at a time
      for (l in electrodeList) { # one electrode at a time
        ...
        # quantify waveform
        peakAmpBin = max(avgWaveform$Amp, na.rm = T)
        peakTemp$PeakAmp[peakTemp$Bin == i & peakTemp$Electrode == l] = peakAmpBin
      }
    }
    dat.sub = rbind(dat.sub, peakTemp)
  }
...
write.table(dat.all, paste(path, "Binned Data/AllSubs_BinnedAverages_P2_peakAmp.txt", sep = ""),sep = "\t", row.names=F)
write.table(BinningInfoTotal, paste(path, "Binned Data/BinningInfo_P2_peakAmp.txt", sep = ""),sep = "\t", row.names=F)
