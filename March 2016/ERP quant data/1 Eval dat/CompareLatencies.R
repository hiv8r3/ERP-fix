...
# Compares latencies for N170s and P2s in eval task
...
# manually determined for each subject

# read in individual file
x = read.delim("ERP quant data/1 Eval dat/Individual quant files/Sub1_P2quant_peak.dat", header = T, sep = "\t") %>%
...
for (i in 2:51) {
  temp = read.delim(paste("ERP quant data/1 Eval dat/Individual quant files/Sub", i, "_P2quant_peak.dat", sep = ""), 
                    header = T, sep = "\t") %>%
        rename(Peak = Amplitude) %>%
        rename(Electrode = Channel)
  temp$Subject = i
...
lmer(Latency ~ Marker + (1|Subject) + (1|Electrode), data = latencyDat) %>%
  summary()
# main effect of marker (P2/N170): P2 is faster (about 8 ms), p = .004

require(ggplot2)
temp = latencyDat[latencyDat$Subject > 46 & latencyDat$Subject < 51,]
