...
electrodeNamesP2=read.delim("./March 2016/ERP quant data/electrodeNames_P2.txt", header = F)
...
k = "AvgMast avgs for ggplot/"
for (j in taskFolderList) {
  for (l in race) {
    for (m in fix) {
        fileName = paste("./March 2016/ERP quant data/", j, k, l, "_", m, "_export.dat", sep="") # generate file name
        ...
        datP2 = rbind(datP2, temp)
    }
  }
}

datP2$Race = as.factor(datP2$Race)
datP2$Fix = as.factor(datP2$Fix)
datP2$Reference = as.factor(datP2$Reference)
datP2$Task = as.factor(datP2$Task)
datP2$Condition = as.factor(datP2$Condition)

# do the same for nose referenced data (doesn't include OZ)
electrodeNamesN170=read.delim("./March 2016/ERP quant data/electrodeNames_N170.txt", header = F)
electrodeNamesN170 = as.character(electrodeNamesN170$V1) # converts to vector of strings

datN170 = NULL

k = "Nose avgs for ggplot/"
for (j in taskFolderList) {
  for (l in race) {
    for (m in fix) {
      fileName = paste("./March 2016/ERP quant data/", j, k, l, "_", m, "_export.dat", sep="") # generate file name
      ...
      temp$Condition = paste(l, m, sep = "_")
      # bind temp data to full data set
      datN170 = rbind(datN170, temp)
    }
  }
}

datN170$Race = as.factor(datN170$Race)
datN170$Fix = as.factor(datN170$Fix)
datN170$Reference = as.factor(datN170$Reference)
datN170$Task = as.factor(datN170$Task)
datN170$Condition = as.factor(datN170$Condition)
...
datN170$avgElec= apply(datN170[,colnames(datN170) %in% aggrList1], 1, FUN=mean)


#Fix Instruction block labels for attend (or at least some of them)
#Fixnames=1:dim(dat)[1]
#Fixnames <- dat$Instr
#Fixnames = as.character(dat$Instr)
#dat.B1$Instr <- Fixnames
# dat$Instr[dat$Instr=="attend"]="Attend"
# dat$Instr[dat$Instr=="suppress"]="Suppress"
# dat$Instr[dat$Instr=="reappraise"]="Reappraise"
# dat$Instr[dat$Instr=="AttendR"]="Attend"
# dat$Instr[dat$Instr=="AttendS"]="Attend"
# 
# dat.b1=dat[!((dat$Study=="Study1") & (dat$Instr %in% c("Suppress", "Reappraise"))),]
# dat.b1=dat.b1[!((dat.b1$Study=="Study2") & (dat.b1$Instr=="Attend")),]
# nrow(dat.b1) == (601*4*1)+(301*4*2) #checking number of rows in new data frame

# #Prime & Target separation
# dat.B1$Prime[dat.B1$Cond %in% c("bt", "bg")]="Black"
# dat.B1$Prime[dat.B1$Cond %in% c("wt", "wg")]="White"
# dat.B1$Target[dat.B1$Cond %in% c("bt", "wt")]="Tool"
# dat.B1$Target[dat.B1$Cond %in% c("bg", "wg")]="Gun"
# 
# #Fix Instruction block labels for attend (or at least some of them)
# FixCNames=1:dim(dat.B1)[1]
# FixCNames <- dat.B1$Cond
# #FixCNames = as.character(dat.B1$Cond)
# FixCNames[dat.B1$Cond %in% c("bt")]="Black+Tool"
# FixCNames[dat.B1$Cond %in% c("bg")]="Black+Gun"
# FixCNames[dat.B1$Cond %in% c("wt")]="White+Tool"
# FixCNames[dat.B1$Cond %in% c("wg")]="White+Gun"
# dat.B1$Cond2 <- FixCNames


# plot it!
# can set y= in aes() call to be composite or any single electrode
require(ggplot2)
require(colorspace)
require(grid)
...
# define plot elements that don't vary first
...
plot.P2.Eval.CZ =
  ggplot(data=datP2[datP2$Task == "1 Eval dat/" ,],
         aes(Time, CZ, group = Condition)) + 
  ERPline + 
  P2box + 
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/P2_Eval_CZ.png", plot.P2.Eval.CZ, width=6, height=4, units="in")


plot.P2.Eval.CPZ =
  # set default parameters in ggplot()
  ggplot(data=datP2[datP2$Task == "1 Eval dat/" ,],
         aes(Time, CPZ, group = Condition)) + 
  ERPline + 
  P2box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/P2_Eval_CPZ.png", plot.P2.Eval.CPZ, width=6, height=4, units="in")


plot.P2.Eval.PZ =
  # set default parameters in ggplot()
  ggplot(data=datP2[datP2$Task == "1 Eval dat/" ,],
         aes(Time, PZ, group = Condition)) + 
  ERPline + 
  P2box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/P2_Eval_PZ.png", plot.P2.Eval.PZ, width=6, height=4, units="in")
...
plot.N170.Eval.TP7 =
  ggplot(data=datN170[datN170$Task == "1 Eval dat/",],
         aes(Time, TP7, group = Condition)) + 
  ERPline + 
  N170box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/N170_Eval_TP7.png", plot.N170.Eval.TP7, width=6, height=4, units="in")

plot.N170.Eval.TP8 =
  ggplot(data=datN170[datN170$Task == "1 Eval dat/",],
         aes(Time, TP8, group = Condition)) + 
  ERPline + 
  N170box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/N170_Eval_TP8.png", plot.N170.Eval.TP8, width=6, height=4, units="in")


plot.N170.Eval.P7 =
  ggplot(data=datN170[datN170$Task == "1 Eval dat/",],
         aes(Time, P7, group = Condition)) + 
  ERPline + 
  N170box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/N170_Eval_P7.png", plot.N170.Eval.P7, width=6, height=4, units="in")

plot.N170.Eval.P8 =
  ggplot(data=datN170[datN170$Task == "1 Eval dat/",],
         aes(Time, P8, group = Condition)) + 
  ERPline + 
  N170box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/N170_Eval_P8.png", plot.N170.Eval.P8, width=6, height=4, units="in")

plot.N170.Eval.avgElec =
  ggplot(data=datN170[datN170$Task == "1 Eval dat/",],
         aes(Time, avgElec, group = Condition)) + 
  ERPline + 
  N170box + 
  # add label for electrode
...
ggsave(filename="./March 2016/ERP quant data/ggplot figures/N170_Eval_avgElec.png", plot.N170.Eval.avgElec, width=6, height=4, units="in")
...
#29A3CC bt  29A3CC
#4DAC26 bg 2EB82E
#92C5DE wt 00CCFF 33D6FF
#B8E186 wg 70DB70


################################
#Adding shading (testing):
