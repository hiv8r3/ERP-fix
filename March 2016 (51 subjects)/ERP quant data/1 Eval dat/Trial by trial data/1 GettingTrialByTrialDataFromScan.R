...
# Calculates average amplitude for P2 for each individual trial to use in trial-by-trial MLM
...
for (k in 1:numSubjects) {
  ...
  numAccepted = nrow(temp.select)/lengthEpoch
  numAccTemp = data.frame(Subject = k,
                          ...
                          numIncorrect = sum(temp.select$Correct == 2)/lengthEpoch,
  ...
  avgTemp = data.frame(Subject = k,
                       Trial = rep(unique(temp.select$Trial), each = length(electrodeList)),
                       Electrode = rep(electrodeList, length(unique(temp.select$Trial)))) 
  # set quant interval for each subject
  intBegin = subInts$Begin[subInts$Subject == k]
  intEnd = subInts$End[subInts$Subject == k]
  
  ####S Begin loop to quantify interval for each trial, for each electrode
  for (i in unique(avgTemp$Trial)) {  # go one trial at a time
    for (l in electrodeList) { # one electrode at a time
      trialTemp = temp.select[temp.select$Trial == i, l]
      avg = mean(trialTemp[intBegin:intEnd])
      avgTemp$MeanAmp[avgTemp$Trial == i & avgTemp$Electrode == l] = avg
    }
  }
  
  # Take out data if electrode is bad
  badElec = read.delim(paste(path, "Eval_Sub", k, "_chan.log", sep=""), header = T)
...
# Note: this data file does not contain information about which condition each trial is. 
# In order to get that, you'll need to use the ev2 file to get that information and pair it up
# with this file by trial. 
write.table(QuantDat, paste(path,"TrialByTrialQuantifiedData.txt", sep=""), sep = "\t", row.names = F)
write.table(NumAcceptedTrials, paste(path,"NumberOfAcceptedTrials.txt", sep=""), sep = "\t", row.names = F)




##### Adds condition information to each trial
QuantDat = read.delim(paste(path,"TrialByTrialQuantifiedData.txt", sep=""), sep = "\t", header = T)
...
write.table(QuantDatCond, paste(path,"TrialByTrialQuantifiedDataWithConditions.txt", sep=""), sep = "\t", row.names = F)
