...
P2dat = read.delim("ERP quant data/2 Cat dat/Quant files/Sub1_P2quant_avgamp.dat", header = T, sep = "\t")
...
for (i in 2:51) {
  temp = read.delim(paste("ERP quant data/2 Cat dat/Quant files/Sub", i, "_P2quant_avgamp.dat", sep = ""), 
                    header = T, sep = "\t")
...
write.table(P2dat, "ERP quant data/2 Cat dat/P2quant_AvgAmp_51subsAggregated.txt", row.names = F, sep = "\t")
...
N170dat = read.delim("ERP quant data/2 Cat dat/Quant files/Sub1_N170quant_peak.dat", header = T, sep = "\t")
N170dat$Subject = 1

# append the rest of the subjects to same data file using a loop
for (i in 2:51) {
  temp = read.delim(paste("ERP quant data/2 Cat dat/Quant files/Sub", i, "_N170quant_peak.dat", sep = ""), 
                    header = T, sep = "\t")
  temp$Subject = i
...
write.table(N170dat, "ERP quant data/2 Cat dat/N170quant_Peak_51subsAggregated.txt", row.names = F, sep = "\t")
